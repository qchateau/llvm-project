...
public:
  ...
  /// Defines how a runWithAST action is affected by other actions.
  enum ASTActionInvalidation {
    /// The request will run unless explicitly cancelled.
    NoInvalidation,
    /// The request will be implicitly delayed by a subsequent update().
    ...
    /// Delaying the request will only execute it for the latest version.
    DelayOnUpdate,
  };
